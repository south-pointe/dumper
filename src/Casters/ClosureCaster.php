    ...
    public function cast(object $var, int $id, int $depth): string
    {
        ...
        if ($file = $ref->getFileName()) {
            ...
            $range = ($startLine !== $endLine)
                ...
                : $startLine;
            return
                $this->decorator->type($var::class . "@{$file}:{$range}") . ' ClosureCaster.php' .
                $this->decorator->comment("#{$id}");
        }

        if ($class = $ref->getClosureScopeClass()) {
            return
                $this->decorator->type("{$class->getName()}::{$ref->getName()}(...)") . ' ClosureCaster.php' .
                $this->decorator->comment("#{$id}");
        }

        return
            $this->decorator->type("{$ref->getName()}(...)") . ' ClosureCaster.php' .
            $this->decorator->comment("#{$id}");
    }
}
